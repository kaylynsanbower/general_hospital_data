...
## Set file paths
########################################################################################
path.code="/Users/kaylynsanbower/Dropbox/Data-Public_Access/HCRIS/data-code"
path.data="/Users/kaylynsanbower/Dropbox/Data-Public_Access/HCRIS/data"


########################################################################################
## Combine different HCRIS versions (1996 and 2010)
########################################################################################
source(paste(path.code,"/uncomp_H1_HCRISv1996.R",sep=""),local=TRUE,echo=FALSE)
source(paste(path.code,"/uncomp_H2_HCRISv2010.R",sep=""),local=TRUE,echo=FALSE)
...
unique.hcris2 = 
  ...
  summarise(beds=max(beds), tot_charges=sum(tot_charges), tot_discounts=sum(tot_discounts),
...
duplicate.hcris3 =
  ...
  mutate_at(c("tot_charges","tot_discounts", "tot_operating_exp", "ip_charges",
              ...
              "tot_uncomp_care_partial_pmts", "new_cap_ass", "cash", "tot_pat_rev", "allowance", "net_pat_rev"),list(~ .*(time_diff/total_days)))
...
## Correct
write_tsv(final.hcris.data,file=paste(path.data,"/extra_vars_HCRIS_Data_thru2019.txt",sep=""),append=FALSE,col_names=TRUE)
